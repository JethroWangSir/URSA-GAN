...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False, no_antialias_up=False, opt=None):
        ...
        if len(self.inject_layers) > 1:
            self.film_layers = nn.ModuleList([FiLM(input_size=256, feature_size=ngf) for _ in range(len(self.inject_layers))])

        self.nse_emb_project_layer = nn.Linear(200 * 768, 256)

    def forward(self, input, layers=[], encode_only=False, nse_emb=None, ch_emb=None, inject_layers=[]):
        if -1 in layers:
            layers.append(len(self.model))
        if nse_emb is not None:
            if not torch.equal(nse_emb, torch.zeros(1, 200, 768, device='cuda')):  # nse_emb != 0
                nse_emb = nse_emb.view(1, -1)  # [1, 200 * 768]
                # print(f'nse_emb before projected: {nse_emb.shape}')
                nse_emb = self.nse_emb_project_layer(nse_emb)
                # print(f'nse_emb after projected: {nse_emb.shape}')
            else:  # nse_emb = 0
                nse_emb = torch.zeros(1, 256, device='cuda')
        ...
        if len(layers) > 0:
            ...
            for layer_id, layer in enumerate(self.model):
                ...
                if layer_id in inject_layers:
                    feat = self.film_layers[film_idx](nse_emb + ch_emb, feat)
                    film_idx += 1
                else:
                    pass
